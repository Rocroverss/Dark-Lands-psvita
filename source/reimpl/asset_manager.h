...
/**
 * [Non-Standard]: Override the APK path used for ZIP-backed asset fallback.
 */
void asset_manager_set_apk_path(const char *path);

/**
 * Android bridge entry point used by native code to unwrap a Java AssetManager.
 * Our loader does not emulate the Java object, so we just return the shared
 * native manager instance.
 */
