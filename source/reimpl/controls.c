...
void controls_init() {
    ...
    int ctrl_ret = sceCtrlSetSamplingModeExt(SCE_CTRL_MODE_ANALOG_WIDE);
    int touch_ret = sceTouchSetSamplingState(SCE_TOUCH_PORT_FRONT, 1);

    // Enable accelerometer
    int motion_ret = sceMotionStartSampling();

    DLA_DEBUG_PRINTF("[controls] init ctrl=0x%08x touch=0x%08x motion=0x%08x\n",
                  ctrl_ret, touch_ret, motion_ret);
}
...
void poll_touch() {
    ...
    if (ret < 0) {
        ...
        if (!warned) {
            warned = 1;
            DLA_DEBUG_PRINTF("[controls][WARN] sceTouchPeek failed: 0x%08x\n", ret);
        }
        return;
    }
