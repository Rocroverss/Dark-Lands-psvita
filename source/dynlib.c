...
// --- Fortify/Bionic and misc shims (weak, libc-friendly) -------------------
#include <stdarg.h>
#include <errno.h>
#include <locale.h>   // uses Vita/newlib's locale_t

#define WEAK __attribute__((weak))

static inline uintptr_t emutls_translate_module_ptr(uintptr_t p);

static void *dynlib_heap_malloc(size_t size) {
#ifdef USE_SCELIBC_IO
    return sceLibcBridge_malloc(size);
#else
    return malloc(size);
#endif
}

static void dynlib_heap_free(void *ptr) {
#ifdef USE_SCELIBC_IO
    sceLibcBridge_free(ptr);
#else
    free(ptr);
#endif
}

static void *dynlib_heap_calloc(size_t num, size_t size) {
#ifdef USE_SCELIBC_IO
    return sceLibcBridge_calloc(num, size);
#else
    return calloc(num, size);
#endif
}

static void *dynlib_heap_realloc(void *ptr, size_t size) {
#ifdef USE_SCELIBC_IO
    return sceLibcBridge_realloc(ptr, size);
#else
    return realloc(ptr, size);
#endif
}

static void *dynlib_heap_memalign(size_t alignment, size_t size) {
#ifdef USE_SCELIBC_IO
    return sceLibcBridge_memalign(alignment, size);
#else
    return memalign(alignment, size);
#endif
}

static int dynlib_heap_posix_memalign(void **memptr, size_t alignment, size_t size) {
    if (!memptr)
        return EINVAL;
    if ((alignment & (alignment - 1)) != 0 || (alignment % sizeof(void *)) != 0)
        return EINVAL;

    void *p = dynlib_heap_memalign(alignment, size);
    if (!p)
        return ENOMEM;
...
static inline int dynlib_ptr_looks_android(uintptr_t p) {
    return (p >= 0x10000000u) && (p < 0x80000000u);
}

static const char *dynlib_safe_cstr(const char *s, const char *who) {
    static int warn_count = 0;
    if (!s)
        return "";

    uintptr_t p = (uintptr_t)s;
    uintptr_t t = emutls_translate_module_ptr(p);
    if (t != p) {
        if (warn_count < 24) {
            DLA_DEBUG_PRINTF("[dynlib][str] %s translated %p -> %p\n", who, s, (void *)t);
            warn_count++;
        }
        return (const char *)t;
    }

    if (dynlib_ptr_looks_android(p)) {
        if (warn_count < 24) {
            DLA_DEBUG_PRINTF("[dynlib][str][WARN] %s unresolved Android pointer %p; using empty string\n", who, s);
            warn_count++;
        }
        return "";
    }
...
static size_t dynlib_strlen(const char *s) { return strlen(dynlib_safe_cstr(s, "strlen")); }
static int dynlib_strcmp(const char *a, const char *b) { return strcmp(dynlib_safe_cstr(a, "strcmp(a)"), dynlib_safe_cstr(b, "strcmp(b)")); }
static int dynlib_strncmp(const char *a, const char *b, size_t n) { return strncmp(dynlib_safe_cstr(a, "strncmp(a)"), dynlib_safe_cstr(b, "strncmp(b)"), n); }
static int dynlib_strcasecmp(const char *a, const char *b) { return strcasecmp(dynlib_safe_cstr(a, "strcasecmp(a)"), dynlib_safe_cstr(b, "strcasecmp(b)")); }
static int dynlib_strncasecmp(const char *a, const char *b, size_t n) { return strncasecmp(dynlib_safe_cstr(a, "strncasecmp(a)"), dynlib_safe_cstr(b, "strncasecmp(b)"), n); }
static char *dynlib_strcpy(char *d, const char *s) { return strcpy(d, dynlib_safe_cstr(s, "strcpy")); }
static char *dynlib_strncpy(char *d, const char *s, size_t n) { return strncpy(d, dynlib_safe_cstr(s, "strncpy"), n); }
static char *dynlib_strcat(char *d, const char *s) { return strcat(d, dynlib_safe_cstr(s, "strcat")); }
static char *dynlib_strncat(char *d, const char *s, size_t n) { return strncat(d, dynlib_safe_cstr(s, "strncat"), n); }
static char *dynlib_strchr(const char *s, int c) { return (char *)strchr(dynlib_safe_cstr(s, "strchr"), c); }
static char *dynlib_strrchr(const char *s, int c) { return (char *)strrchr(dynlib_safe_cstr(s, "strrchr"), c); }
static char *dynlib_strstr(const char *h, const char *n) { return (char *)strstr(dynlib_safe_cstr(h, "strstr(h)"), dynlib_safe_cstr(n, "strstr(n)")); }
static size_t dynlib_strnlen(const char *s, size_t n) { return strnlen(dynlib_safe_cstr(s, "strnlen"), n); }
static char *dynlib_strdup(const char *s) {
    const char *safe = dynlib_safe_cstr(s, "strdup");
    size_t len = strlen(safe) + 1;
    char *copy = (char *)dynlib_heap_malloc(len);
    if (!copy)
        return NULL;
    memcpy(copy, safe, len);
    return copy;
}
static size_t dynlib_strlcpy(char *d, const char *s, size_t n) { return strlcpy(d, dynlib_safe_cstr(s, "strlcpy"), n); }
static size_t dynlib_strlcat(char *d, const char *s, size_t n) { return strlcat(d, dynlib_safe_cstr(s, "strlcat"), n); }

// *_chk wrappers: ignore size guard and forward to the plain function.
WEAK size_t __strlen_chk(const char *s, size_t bufsize) { (void)bufsize; return dynlib_strlen(s); }
WEAK void *__memcpy_chk(void *d, const void *s, size_t n, size_t bufsize) { (void)bufsize; return memcpy(d,s,n); }
WEAK void *__memset_chk(void *s, int c, size_t n, size_t bufsize) { (void)bufsize; return memset(s,c,n); }
WEAK char *__strcpy_chk(char *d, const char *s, size_t bufsize) { (void)bufsize; return dynlib_strcpy(d,s); }
WEAK char *__strcat_chk(char *d, const char *s, size_t bufsize) { (void)bufsize; return dynlib_strcat(d,s); }
WEAK char *__strncpy_chk(char *d, const char *s, size_t n, size_t bufsize) { (void)bufsize; return dynlib_strncpy(d,s,n); }
WEAK char *__strchr_chk(const char *s, int c, size_t bufsize) { (void)bufsize; return dynlib_strchr(s,c); }
WEAK char *__strrchr_chk(const char *s, int c, size_t bufsize) { (void)bufsize; return dynlib_strrchr(s,c); }
WEAK ssize_t __read_chk(int fd, void *buf, size_t n, size_t bufsize) { (void)bufsize; return read_soloader(fd, buf, n); }

// Android-style fortify vararg wrappers
WEAK int __vsprintf_chk(char *dst, int flag, size_t dstlen,
                        const char *fmt, va_list ap) {
    (void)flag; (void)dstlen;
    return vsprintf(dst, fmt, ap);
}
WEAK int __vsnprintf_chk(char *dst, size_t n, int flag, size_t dstlen,
                         const char *fmt, va_list ap) {
    (void)flag; (void)dstlen;
    return vsnprintf(dst, n, fmt, ap);
}

// getauxval: return 0 when not available
WEAK unsigned long getauxval(unsigned long type) { (void)type; return 0UL; }

// __register_atfork: accept and return 0 (no-op)
WEAK int __register_atfork(void (*prepare)(void),
                           void (*parent)(void),
                           void (*child)(void)) {
...
// FD_* fortify wrappers (checked variants)
WEAK void __FD_SET_chk(int fd, fd_set *set, size_t sz) { (void)sz; FD_SET(fd, set); }
WEAK void __FD_CLR_chk(int fd, fd_set *set, size_t sz) { (void)sz; FD_CLR(fd, set); }
WEAK int  __FD_ISSET_chk(int fd, fd_set *set, size_t sz) { (void)sz; return FD_ISSET(fd, set); }

// Map nrt* to rt* equivalents (weak so libc wins if present)
WEAK size_t wcsnrtombs(char *dst, const wchar_t **src, size_t nwc,
                       size_t len, mbstate_t *ps) {
    (void)nwc;
    return wcsrtombs(dst, src, len, ps);
}
WEAK size_t mbsnrtowcs(wchar_t *dst, const char **src, size_t nmc,
                       size_t len, mbstate_t *ps) {
    (void)nmc;
    return mbsrtowcs(dst, src, len, ps);
}

// *_l locale variants: weak wrappers to non-localized versions (libc may provide)
WEAK long long strtoll_l(const char *__restrict nptr, char **__restrict endptr, int base, locale_t loc) {
    (void)loc; return strtoll(nptr, endptr, base);
}
WEAK unsigned long long strtoull_l(const char *__restrict nptr, char **__restrict endptr, int base, locale_t loc) {
    (void)loc; return strtoull(nptr, endptr, base);
}
WEAK long double strtold_l(const char *__restrict nptr, char **__restrict endptr, locale_t loc) {
    (void)loc; return strtold(nptr, endptr);
}

// Per-thread locale APIs: stubbed (weak, so real ones win if present)
WEAK locale_t uselocale(locale_t newloc) { (void)newloc; return (locale_t)0; }
WEAK locale_t newlocale(int category_mask, const char *locale, locale_t base) {
    (void)category_mask; (void)locale; (void)base; return (locale_t)0;
}
WEAK void freelocale(locale_t loc) { (void)loc; }

// Provide posix_memalign (some Vita toolchains lack it)
WEAK int posix_memalign(void **memptr, size_t alignment, size_t size) {
    if ((alignment & (alignment - 1)) != 0 || (alignment % sizeof(void*)) != 0) return EINVAL;
...
// -----------------------------------------------------------------------------
// Minimal GCC emutls support (Android/NDK thread_local)
//
// Android NDK toolchains may emit calls to __emutls_get_address() to implement
// thread_local storage. The runtime implementation normally lives in libgcc /
// compiler-rt. If the loaded .so expects it as an external symbol, we must
// provide it.
//
// This implementation is intentionally minimal: it allocates one backing
// instance per TLS object for the entire process (i.e., not per-thread). This
// is sufficient for many single-threaded titles and prevents crashes during
// static initialization.
// -----------------------------------------------------------------------------

static inline int emutls_ptr_in_so_range(uintptr_t p) {
    uintptr_t v = p & ~(uintptr_t)1;
    uintptr_t text_lo = (uintptr_t)so_mod.text_base;
    uintptr_t text_hi = text_lo + (uintptr_t)so_mod.text_size;
    if (v >= text_lo && v < text_hi)
        return 1;

    for (int i = 0; i < so_mod.n_data; i++) {
        uintptr_t data_lo = (uintptr_t)so_mod.data_base[i];
        uintptr_t data_hi = data_lo + (uintptr_t)so_mod.data_size[i];
        if (v >= data_lo && v < data_hi)
            return 1;
    }
...
// Convert potential Android-era absolute pointers (0x31xxxxxx base) or raw
// ELF VADDRs to runtime pointers in the mapped module.
static inline uintptr_t emutls_translate_module_ptr(uintptr_t p) {
    ...
    if (emutls_ptr_in_so_range(p0))
        return p;

    {
        const uintptr_t text_start = (uintptr_t)so_mod.text_start;
        const uintptr_t text_end = text_start + (uintptr_t)so_mod.text_size;
        if (p0 >= text_start && p0 < text_end) {
            return ((uintptr_t)so_mod.text_base + (p0 - text_start)) | thumb;
        }
        for (unsigned b = 0; b < (unsigned)(sizeof(android_bases) / sizeof(android_bases[0])); b++) {
            uintptr_t android_base = android_bases[b];
            if (p0 >= android_base + text_start && p0 < android_base + text_end) {
                return ((uintptr_t)so_mod.text_base + (p0 - (android_base + text_start))) | thumb;
            }
        }
    }

    for (int i = 0; i < so_mod.n_data; i++) {
        const uintptr_t seg_start = (uintptr_t)so_mod.data_start[i];
        const uintptr_t seg_end = (uintptr_t)so_mod.data_end[i];
        if (p0 >= seg_start && p0 < seg_end) {
            return ((uintptr_t)so_mod.data_base[i] + (p0 - seg_start)) | thumb;
        }
        for (unsigned b = 0; b < (unsigned)(sizeof(android_bases) / sizeof(android_bases[0])); b++) {
            uintptr_t android_base = android_bases[b];
            if (p0 >= android_base + seg_start && p0 < android_base + seg_end) {
                return ((uintptr_t)so_mod.data_base[i] + (p0 - (android_base + seg_start))) | thumb;
            }
        }
    }
...
static inline const void *emutls_resolve_template_ptr(const __emutls_object *obj) {
    // Canonical compiler-rt/libgcc layout keeps template at +0xC.
    uintptr_t templ = emutls_translate_module_ptr((uintptr_t)obj->templ);
    if (templ && emutls_ptr_in_so_range(templ))
        return (const void *)templ;

    // Some toolchains/older code may treat +0x8 as template and +0xC as storage.
    uintptr_t legacy_templ = emutls_translate_module_ptr((uintptr_t)obj->object);
    if (legacy_templ && emutls_ptr_in_so_range(legacy_templ))
        return (const void *)legacy_templ;

    return NULL;
}

WEAK void *__emutls_get_address(void *obj_ptr) {
    __emutls_object *obj = (__emutls_object *)obj_ptr;
    if (!obj) {
        return NULL;
    }

    // Do not write back into the descriptor (obj->object).
    // On some builds the descriptor can live in read-only memory (e.g. RELRO),
    // and any store will trigger a data abort on Vita.
    //
    // Instead, keep a small process-wide map from descriptor address -> storage.
    // NOTE: This is not per-thread TLS; it is sufficient for most libc++/libstdc++
    // code paths used by this project.
    typedef struct emutls_node {
        const __emutls_object *key;
        void *storage;
        struct emutls_node *next;
    } emutls_node;

    static emutls_node *g_emutls_list = NULL;

    for (emutls_node *n = g_emutls_list; n; n = n->next) {
        if (n->key == obj) {
            return n->storage;
        }
    }
    ...
    void *storage = NULL;
    // Align must be a power of two. Fall back to 16 if the descriptor is malformed.
    size_t align = (obj->align && !(obj->align & (obj->align - 1))) ? obj->align : 16;
    if (posix_memalign(&storage, align, obj->size ? obj->size : 1) != 0) {
        return NULL;
    }

    const void *templ = emutls_resolve_template_ptr(obj);
    if (templ && obj->size) {
        memcpy(storage, templ, obj->size);
    } else if (obj->size) {
        memset(storage, 0, obj->size);
    }

    emutls_node *node = (emutls_node *)malloc(sizeof(emutls_node));
    if (!node) {
        free(storage);
        return NULL;
    }
...
// Some Android NDK builds use __cxa_thread_atexit_impl to register thread_local
// destructors. This port does not rely on thread-exit teardown, so we simply
// report success.
WEAK int __cxa_thread_atexit_impl(void (*dtor)(void *), void *obj, void *dso_handle) {
    (void)dtor;
    (void)obj;
    (void)dso_handle;
    return 0;
...
void __cxa_new_handler(void)
{
    // operator new exhausted the SceLibc heap.  Returning here would cause
    // libc++ to retry the allocation indefinitely (ABI contract: return = retry).
    // Instead, print a clear diagnostic and trap so the coredump points directly
    // at the allocation site rather than looping forever or crashing elsewhere.
    // If you see this, raise sceLibcHeapSize in main.c.
    void *lr;
    __asm__ volatile ("mov %0, lr" : "=r"(lr));
    sceClibPrintf("[FATAL] __cxa_new_handler: SceLibc heap OOM (operator new failed). "
                  "LR=%p  Raise sceLibcHeapSize in main.c.\n", lr);
    __builtin_trap();
}

// Used both for imported operator-new symbols and for the inline hook installed
// over the .so's own operator new implementation.
void *operator_new_guarded(unsigned int size)
{
    void *caller_lr = NULL;
    __asm__ volatile ("mov %0, lr" : "=r"(caller_lr));
    if (size == 0)
        size = 1;
    void *p = dynlib_heap_malloc(size);
    if (!p) {
        sceClibPrintf("[FATAL] operator new(%u) failed (invalid size or OOM). caller=%p\n",
                      size, caller_lr);
        __builtin_trap();
    }
    return p;
...
void *operator_new_array_guarded(unsigned int size)
{
    void *caller_lr = NULL;
    __asm__ volatile ("mov %0, lr" : "=r"(caller_lr));
    if (size == 0)
...
void operator_delete_guarded(void *ptr)
{
    dynlib_heap_free(ptr);
}

void operator_delete_array_guarded(void *ptr)
{
    dynlib_heap_free(ptr);
}

so_default_dynlib default_dynlib[] = {
        { "__cxa_new_handler", (uintptr_t)&__cxa_new_handler},
        { "__cxa_thread_atexit_impl", (uintptr_t)&__cxa_thread_atexit_impl},
        { "__emutls_get_address", (uintptr_t)&__emutls_get_address},
        //new imports -> { "fff", (uintptr_t)&fff2 },
        // --- Fortify/Bionic *_chk --------------------------------------------
        ...
        // *_l variants mapped to non-localized functions
        { "strtoll_l",   (uintptr_t)&strtoll_l },
        { "strtoull_l",  (uintptr_t)&strtoull_l },
        { "strtold_l",   (uintptr_t)&strtold_l },

        // --- Memory helpers ---------------------------------------------------
        { "posix_memalign", (uintptr_t)&dynlib_heap_posix_memalign },
        ...
        // --- FILE* singletons imported by name --------------------------------
        ...
        { "_ZdaPv", (uintptr_t)&operator_delete_array_guarded },
        { "_ZdlPv", (uintptr_t)&operator_delete_guarded },
        { "_Znaj", (uintptr_t)&operator_new_array_guarded },
        { "_Znwj", (uintptr_t)&operator_new_guarded },
        ...
        // Memory
        { "calloc", (uintptr_t)&dynlib_heap_calloc },
        { "free", (uintptr_t)&dynlib_heap_free },
        { "malloc", (uintptr_t)&dynlib_heap_malloc },
        { "memalign", (uintptr_t)&dynlib_heap_memalign },
        ...
        { "realloc", (uintptr_t)&dynlib_heap_realloc },
        ...
        { "strcasecmp", (uintptr_t)&dynlib_strcasecmp },
        { "strcat", (uintptr_t)&dynlib_strcat },
        { "strchr", (uintptr_t)&dynlib_strchr },
        { "strcmp", (uintptr_t)&dynlib_strcmp },
        { "strcoll", (uintptr_t)&strcoll },
        { "strcpy", (uintptr_t)&dynlib_strcpy },
        { "strcspn", (uintptr_t)&strcspn },
        { "strdup", (uintptr_t)&dynlib_strdup },
        { "strlcat", (uintptr_t)&dynlib_strlcat },
        { "strlcpy", (uintptr_t)&dynlib_strlcpy },
        { "strlen", (uintptr_t)&dynlib_strlen },
        { "strncasecmp", (uintptr_t)&dynlib_strncasecmp },
        { "strncat", (uintptr_t)&dynlib_strncat },
        { "strncmp", (uintptr_t)&dynlib_strncmp },
        { "strncpy", (uintptr_t)&dynlib_strncpy },
        { "strnlen", (uintptr_t)&dynlib_strnlen },
        { "strpbrk", (uintptr_t)&strpbrk },
        { "strrchr", (uintptr_t)&dynlib_strrchr },
        { "strspn", (uintptr_t)&strspn },
        { "strstr", (uintptr_t)&dynlib_strstr },
