...
#ifndef DLA_GXM_MULTISAMPLE_MODE
#define DLA_GXM_MULTISAMPLE_MODE SCE_GXM_MULTISAMPLE_NONE
#endif
...
void gl_preload() {
    ...
    // DUMP_COMPILED_SHADERS writes translated GXP binaries after first compile.
    // If the directory does not exist, every boot pays the GLSL translation cost
    // again. Create it before any shader can be compiled.
    file_mkpath(DATA_PATH "gxp/.keep", 0777);
#endif
...
void gl_init() {
    vglInitExtended(0, 960, 544, 6 * 1024 * 1024, DLA_GXM_MULTISAMPLE_MODE);
}
...
void glPixelStorei_soloader(GLenum pname, GLint param) {
    ...
    case GL_PACK_ALIGNMENT:
        // VitaGL may reject alignment enums that the game sets routinely.
        // Keep the render path moving; the common/default value is already 4.
        return;
...
#ifdef DEBUG_OPENGL
    DLA_DEBUG_PRINTF("[gl_dbg] glShaderSource<%p>(shader: %i, count: %i, string: %p, length: %p)\n", __builtin_return_address(0), shader, count, string, _length);
#endif
    if (!string) {
...
#ifdef DEBUG_OPENGL
    DLA_DEBUG_PRINTF("[gl_dbg] glCompileShader<%p>(shader: %i)\n", __builtin_return_address(0), shader);
#endif
